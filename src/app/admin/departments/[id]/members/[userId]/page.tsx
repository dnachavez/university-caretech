...
export default function DepartmentMemberDetailPage({ 
  params 
}: { 
  ...
  useEffect(() => {
    ...
    // Fetch user details
    fetchUserDetails()
  }, [isAuthenticated, currentUser, router, params.id, params.userId])
  ...
  const fetchDepartment = async () => {
    try {
      ...
      setError(typeof err === 'object' && err !== null ? (err as Error).message : "Failed to load department information")
    }
  }
    ...
    return (
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-gray-700">Member Details</h1>
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex items-center text-sm text-gray-500 mb-6 -mt-4">
        ...
        <ChevronRight className="h-4 w-4 mx-1" />
        <Link href={`/admin/departments/${params.id}`} className="hover:text-blue-600">
          {department.name}
        </Link>
        <ChevronRight className="h-4 w-4 mx-1" />
