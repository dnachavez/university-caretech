...
export async function POST(req: NextRequest) {
  try {
    ...
    const existingForm = await prisma.studentHealthForm.findUnique({
      where: { userId }
    })
    ...
    if (existingForm) {
      ...
      healthForm = await prisma.studentHealthForm.update({
        where: { userId },
        data: {
          lastName,
          firstName,
      ...
      // Create new form
      healthForm = await prisma.studentHealthForm.create({
        data: {
          userId,
          lastName,
          firstName,
...
export async function GET(req: NextRequest) {
  try {
    ...
    const healthForm = await prisma.studentHealthForm.findUnique({
      where: { userId }
    })
